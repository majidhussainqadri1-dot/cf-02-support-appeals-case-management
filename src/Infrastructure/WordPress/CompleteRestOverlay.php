...
final class CompleteRestOverlay
{
    ...
    private function casePage(\WP_REST_Request $request,PrincipalContext $context,string $kind,array $requesterRefs,array $filters,bool $staff):array
    {
        ...
        if($staff&&$context->hasAnyCapability('queue.manage','audit.sample.read')){$scopes=$context->queueScopes();if($scopes===[])throw new RuntimeException('Scoped queue authority is required.');$clauses[]='c.queue_key IN ('.implode(',',array_fill(0,count($scopes),'%s')).')';array_push($args,...$scopes);}elseif($staff){$clauses[]='EXISTS (SELECT 1 FROM '.$wpdb->prefix.'cf02_assignments a WHERE a.case_uuid=c.case_uuid AND a.agent_ref=%s AND a.ended_at IS NULL)';$args[]=$context->actorReference();}

...
final class RepairService
{
    public static function register():void
    {
        add_filter('site_status_tests',static function(array $tests):array{
            $tests['direct']['cf02_repair_integrity']=['label'=>__('CF-02 installation integrity','cf-02-support-appeals-case-management'),'test'=>static function():array{
                $i=self::inspect();$healthy=$i['schema_defects']===[]&&$i['legacy_local_roles_present']===[]&&$i['route_receipt_errors']===[];
    ...
    {
        global $wpdb;$schemaDefects=Installer::schemaDefects((string)$wpdb->prefix);
        $missing=array_values(array_map(
            static fn(string $defect):string=>substr($defect,0,-strlen(':missing-table')),
            array_filter($schemaDefects,static fn(string $defect):bool=>str_ends_with($defect,':missing-table'))
        ));
        $receipts=get_option('cf02_route_contract_receipts',[]);$receipts=is_array($receipts)?$receipts:[];
        return ['schema_expected'=>SchemaCompletion::VERSION,'schema_installed'=>Installer::schemaVersion(),'schema_defects'=>$schemaDefects,'missing_tables'=>$missing,'route_receipt_errors'=>RouteRegistrar::validateReceipts($receipts),'legacy_local_roles_present'=>self::legacyRoles(),'scheduler'=>Scheduler::inspection(),'repairable'=>true,'destructive'=>false];
    }
    ...
    public static function repair(PrincipalContext $context,string $approvalRef,DateTimeImmutable $at):array
    {
        ...
        $result['status']=$result['schema_defects']===[]&&$result['legacy_local_roles_present']===[]&&$result['route_receipt_errors']===[]?'repaired':'incomplete';

...
final class RequiredCompanionContracts
{
    ...
    public static function validate(array $contracts): array
    {
        ...
        foreach (self::DEFINITIONS as $key => $definition) {
            ...
            $required = $definition['required'] || (is_array($contract) && ($contract['enabled'] ?? false) === true);
            if (!$required && (!is_array($contract) || ($contract['enabled'] ?? false) !== true)) {
                continue;
            }
            ...
            $health = $contract['health_checked_at'] ?? null;
            if (!is_string($health) || strtotime($health) === false || strtotime($health) < time() - 900) {

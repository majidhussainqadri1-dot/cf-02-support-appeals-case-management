...
$test('idempotency preserves exact requester identity', static function (): void {
    assert(IdempotencyKey::forIntake(IntakeChannel::Web, 'User-A', 'message-1')->value()
        !== IdempotencyKey::forIntake(IntakeChannel::Web, 'user-a', 'message-1')->value());
});
...
$test('message idempotency collision with changed payload is rejected', static function (): void {
    ...
    $thrown = false;
    try { $case->appendMessage($changed, 2); } catch (DomainException) { $thrown = true; }
    assert($thrown);
});
...
$test('attachment identifier collision is rejected but exact replay is idempotent', static function (): void {
    ...
    $thrown = false;
    try { $case->addAttachment($changed, 2); } catch (DomainException) { $thrown = true; }
    assert($thrown);
});
...
$test('requester projection hides quarantined and requester-hidden attachments', static function (): void {
    ...
    $hiddenHash = hash('sha256', 'h');
    $hidden = new AttachmentRecord('att-h', $caseId, $hiddenHash, 'text/plain', 10, 'internal diagnostic', new DateTimeImmutable(), 'private://h', 'C2', false, false);
    $hidden->transition(AttachmentState::Quarantined);
    $hidden->recordScan(new AttachmentScanResult($hiddenHash, 'text/plain', 'clean', 'scanner', '1.0.0', new DateTimeImmutable()));
    $hidden->transition(AttachmentState::Available);
    assert($case->addAttachment($quarantined, 1));
    assert($case->addAttachment($hidden, 2));
    assert(UserCaseProjection::fromWorkspace($case)['attachment_ids'] === []);
});
...
$test('direct resolved and closed state bypass is rejected', static function (): void {
    ...
    $thrown = false;
    try { $case->transition(CaseState::Resolved, 3); } catch (DomainException) { $thrown = true; }
    assert($thrown);
});
...
$test('governed resolution and closure require verified outcome and notice', static function (): void {
    ...
    $case->transition(CaseState::Triaged, 1);
    $case->transition(CaseState::InProgress, 2);
    $decision = new ResolutionDecision(ResolutionCode::InformationProvided, ['Provided bounded troubleshooting steps'], null,
        'Please verify the corrected behavior.', true, new DateTimeImmutable('+7 days'), true, true);
...
$test('receipt shows unverified sender trust without granting authority', static function (): void {
    $request = new IntakeRequest('user-1', IntakeChannel::Email, 'mail-2', false, 'technical',

...
$technicalRequest = static fn (string $source = 'web-001', bool $verified = true): IntakeRequest => new IntakeRequest(
    ...
    [
        'route' => '/support/new',
        'device_context' => 'Chrome 151 on Windows 10',
        'reproduction_steps' => 'Open support, choose Technical Fault, submit the form.',
    ],
    'medium',
    'normal',
    'ur-PK',
...
$test('guided intake rejects passwords OTPs and payment-card data', static function (): void {
    ...
    try {
        new IntakeRequest(
            ...
            [
                'route' => '/support/new',
                'device_context' => 'Browser',
                'reproduction_steps' => 'Submit the form.',
            ],
            'medium',
            'normal',
            'en-US'
...
$test('privacy intake remains specialist-only and purpose-bound', static function (): void {
    $request = new IntakeRequest(
        ...
        [
            'request_type' => 'access',
            'identity_verification_reference' => 'File00-verification-ref-1',
            'scope' => 'account and profile data',
        ],
        'high',
        'urgent',
        'en-US'
...
$test('case workspace rejects stale writes and foreign-case attachments', static function (): void {
    ...
    $stale = false;
    try {
        $case->clearBlocker('block-1', 1);
    } catch (ConcurrencyConflict) {
        $stale = true;
    }
    assert($stale);

    $foreign = new AttachmentRecord(
        'att-foreign',
        ...
        'private://attachments/foreign',
        'C2'
    );

    $rejected = false;
    try {
        $case->addAttachment($foreign, 2);
    } catch (InvalidArgumentException) {
        $rejected = true;
    }
    assert($rejected);
...
$test('attachment remains inaccessible before scan and across case boundary', static function (): void {
    $caseId = SupportCaseId::generate();
    $attachment = new AttachmentRecord(
        'att-1',
        $caseId,
        hash('sha256', 'safe-file'),
        'image/png',
        1024,
        'screen capture for technical diagnosis',
        new DateTimeImmutable('2026-08-03T19:00:00+05:00'),
        'private://attachments/att-1',
        'C2'
    );

    assert($attachment->downloadReference($caseId, 'requester') === null);
    $attachment->transition(AttachmentState::Quarantined);
    $attachment->transition(AttachmentState::Scanned);
...
$test('C4 evidence requires specialized vault reference', static function (): void {
    ...
    try {
        new AttachmentRecord(
            'att-c4',
            SupportCaseId::generate(),
            hash('sha256', 'identity evidence'),
            'application/pdf',
            2048,
            'identity verification evidence',
            new DateTimeImmutable(),
            'private://ordinary/att-c4',
            'C4',
            false
        );
    } catch (InvalidArgumentException) {
...
$test('deduplication forbids cross-requester merge and supports verified reversal', static function (): void {
    $crossRequester = false;
    try {
        new CaseMergePlan(SupportCaseId::generate(), SupportCaseId::generate(), 'user-1', 'user-2', 'Probable replay', hash('sha256', 'preview'));
    } catch (InvalidArgumentException) {
        $crossRequester = true;
    }
    ...
    $mismatch = false;
    try {
        $plan->apply(hash('sha256', 'changed-preview'));
...
$test('receipt never claims provider delivery when provider is unavailable', static function () use ($technicalRequest): void {
    $request = new IntakeRequest(
        'user-100',
        IntakeChannel::Email,
        'mail-001',
        ...
        [
            'route' => '/support/new',
            'device_context' => 'Email client and Chrome',
            'reproduction_steps' => 'Open support and submit.',
        ],
        'medium',
        'normal',
        'en-US'
...
$test('resolution law blocks open blockers failed native commands and silent auto-close', static function (): void {
    $decision = new ResolutionDecision(
        ResolutionCode::NativeOwnerActionCompleted,
        ['Requested native owner action', 'Verified resulting state'],
        'File21:decision:100:v2',
        'Please confirm that the corrected publication state is now visible.',
        true,

...
$releaseManifest=json_decode((string)file_get_contents(dirname(__DIR__).'/release/manifest.json'),true,512,JSON_THROW_ON_ERROR);
$identity=['runtime_version'=>(string)$releaseManifest['plugin_version'],'schema_version'=>(string)$releaseManifest['database_schema_version'],'source_sha'=>str_repeat('a',40),'package_sha256'=>str_repeat('b',64)];
